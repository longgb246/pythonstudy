...
class inventory_proess:
    def __init__(self, fdc_forecast_sales, fdc_forecast_std, fdc_alt, fdc_alt_prob, fdc_inv, white_list_dict,
                 fdc_allocation, fdc, rdc_inv, order_list, date_range, orders_retail, all_sku_list):
        ''' #类初始化函数，初始化类的各个参数'''
    ...
    def calc_fdc_allocation(self, date_s, fdc):
        ...
        for s in self.white_list:
            ...
            while c < alt:
                ...
                self.fdc_inv[index_tmp]['open_po'] = self.fdc_inv[index_tmp]['open_po'] + self.fdc_inv[index_tmp][
                    'open_po']
            ...
            # 更新当日到达量
            self.fdc_inv[index_1]['arrive_quantity'] = self.fdc_inv[index]['allocation'] + self.fdc_inv[index_1][
                'arrive_quantity']
    ...
    def OrdersSimulation(self):
        for d in self.date_range:
            ...
            for f in self.fdc:
                ...
                for o in self.orders_retail.items():
                    # 遍历订单,尽量按照时间顺序进行遍历
                        ...
                        # 在白名单，但是fdc货不够发
                            ...
                            if s[1] > self.rdc_inv[index_rdc]:
                                if s[1] > self.rdc_inv[index_rdc] + self.fdc_inv[index]['inv']:
                                    # print 'what happened'
                                    self.simu_orders_retail[index] = self.rdc_inv[index_rdc] + self.fdc_inv[index][
                                        'inv']
                                    tmp['fdc_rdc'] = 1
                                else:
                                    tmp['fdc_rdc'] = 1
