...
class inventory_proess:
    ...
    def save_result(self):
        pickle.dump(self.fdc_inv,open(self.save_data_path+'fdc_inv','w'))
        #pickle.dump(white_list_dict,open(save_data_path+'white_list_dict','w'))
        pickle.dump(self.fdc_allocation,open(self.save_data_path+'fdc_allocation','w'))
        pickle.dump(self.rdc_inv,open(self.save_data_path+'rdc_inv','w'))
        pickle.dump(self.order_list,open(self.save_data_path+'order_list','w'))
        pickle.dump(self.orders_retail,open(self.save_data_path+'orders_retail','w'))
        pickle.dump(self,open(self.save_data_path+'all_sku_list','w'))
                    ...
                    for s in o[1].items():
                        ...
                        if s[0] not in self.white_list[f]:
                            ...
                            if s[1]>self.rdc_inv[index_rdc]:
                                #print 'what happened'
                                self.simu_orders_retail[f+d][order_index]=self.rdc_inv[index_rdc]
                        ...
                        #在白名单，但是fdc货不够发
                            ...
                            if s[1]>self.rdc_inv[index_rdc]:
                                if s[1]>self.rdc_inv[index_rdc]+self.fdc_inv[index]['inv']:
                                    #print 'what happened'
                                    self.simu_orders_retail[f+d][order_index]=self.rdc_inv[index_rdc]+self.fdc_inv[index]['inv']
