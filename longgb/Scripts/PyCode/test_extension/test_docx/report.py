...
class SimReport:
    page = ''
    def __init__(self, report_name, sort_id, period, file_path):
    ...
    def build_head(self):
        self.page = PyH('simulation report')
        self.page.__iadd__('<meta http-equiv="Content-Type" content="text/html; charset=utf-8">')
        self.page.__iadd__('''
		<style>
		<!--
		.mytable{
    ...
    def build_overview(self, datas):
        self.page << h1('模拟分析报告（'+ str(self.sort_id) + '）')
        self.page << p('分析时间： '+ self.period )
        self.page << p('SKU总数：' + str(datas["total"]))
        self.page << p('SKU上架时间早于'+self.period[:10]+'的SKU数：'+ str(datas["screened"]))

    def build_analysis(self):
        self.page << h2('KPI相关的图')
        table_analysis = self.page << table(cl='mytable',id='analysis_table')

        tr1 = table_analysis << tr()
        tr1 << self.__build_graphtd__('cr.PNG', '服务水平 = 库存（inv>0）天数 / 统计天数')
        tr1 << self.__build_graphtd__('td.PNG', '周转天数=平均库存/平均销量')
        tr1 << self.__build_graphtd__('cr_td.PNG', '')

        tr2 = table_analysis << tr()
        tr2 << self.__build_graphtd__('z.PNG', '')
        tr2 << self.__build_graphtd__('bp.PNG', '')
        tr2 << self.__build_graphtd__('vlt.PNG', '')

        tr3 = table_analysis << tr()
        tr3 << self.__build_graphtd__('actual_origin_rate.PNG', '')
        tr3 << self.__build_graphtd__('actual_plan_rate.PNG', '')

    def  __build_graphtd__(self, graph, describe=''):
        td1 = td()
        td1.attributes['width'] = '30%'  # 宽度30%，为了适应
        img1 = img()
        img1.attributes['src'] = graph
    ...
    def build_sim_rst(self):
        self.page << h2('模拟结果')
        self.page << img(src='inv.PNG')

    def build_kpis(self, kpis):
        table1 = self.page << table(cl='mytable',id='kpi_table')
        tr1 = table1 << tr()
        tr1 << self.__build_th_row__(2,'仿真类型')
        tr1 << self.__build_th_col__(2,'现货率')
        tr1 << self.__build_th_col__(2,'周转')
        tr1 << self.__build_th_row__(2,'GMV')
        tr1 << self.__build_th_row__(2,'补货次数')
        tr1 = table1 << tr()
        tr1<< th('简单平均')
        tr1<< th('销量加权平均')
        tr1<< th('平均库存/平均销量')
        ...
        for sim_kpi in kpis.T.values:
            tr1 = table1 << tr()
            for item in sim_kpi:
                tr1 << td(item)

    def  __build_th_col__(self, colspan, describe=''):
        td1 = th(describe)
        td1.attributes['colspan'] = colspan
        return td1
    def  __build_th_row__(self, rowspan, describe=''):
        td1 = th(describe)
        td1.attributes['rowspan'] = rowspan
        return td1

    def build_replenishment_sample(self):
        self.page << h2('补货案例分析')
        self.page << h3(src='inv.PNG')
        self.page << h3('低周转，高现货')
        low_td_high_cr_sample = os.listdir(self.file_path+'\\high_cr_low_td')[0]
        self.page << img(src='.\\high_cr_low_td\\'+ low_td_high_cr_sample)
        self.page << h3('高周转，高现货')
        high_td_high_cr_sample = os.listdir(self.file_path+'\\high_cr_high_td')[0]
        self.page << img(src='.\\high_cr_high_td\\'+ high_td_high_cr_sample)
        self.page << h3('高周转，低现货')
        high_td_low_cr_sample = os.listdir(self.file_path+'\\low_cr_high_td')[0]
        self.page << img(src='.\\low_cr_high_td\\'+ high_td_low_cr_sample)
        self.page << h3('低周转，低现货')
        low_td_low_cr_sample = os.listdir(self.file_path+'\\low_cr_low_td')[0]
        self.page << img(src='.\\low_cr_low_td\\'+ low_td_low_cr_sample)

    def print_report(self, datas):
        self.build_head()
        self.build_overview(datas)
        self.build_analysis()
        #self.build_sim_rst()
        #self.build_kpis(kpis)
        self.build_replenishment_sample()
        self.page.printOut(self.file_path + '\index.html')
...
class ReportWord:
    ...
    def kpianalysis(self, kpi_cr_str, kpi_ito_str, sim_date_range):
        ...
        temp_text2 = u'a'
        self.skuget(u'high_cr_high_td', u'（2）现货率高，周转高的SKU', u'\t这部分SKU存在降低库存周转的空间，尤其在现货率接近1的SKU。下图展示了几个比较典型的SKU的库存情况。下面给出几个此类SKU代表。', u'\t在这个区域的SKU，库存水平（相对销量）始终保持在较高的水平。由库存水平可以看出，周转高的主要原因是：（a）单次采购量过大。（b）采购时间过早。\n\t对于这部分SKU，可以采用降低单次采购量，保持高服务水平的情况下多次采购来降低周转。如果适当推迟补货时间，现货率仍然会维持在100%（或接近100%），同时库存周转天数可以大幅降低。')
        self.skuget(u'low_cr_low_td', u'（3）现货率低，周转低的SKU', u'\t在同样库存周转天数的情况下，这部分SKU的现货率明显低于位于（1）的SKU（现货率高，周转低的SKU）。下面给出几个此类SKU代表。', u'\t导致这类SKU现货率较低/周转天数长的原因为：\n\t（a）采购时间把握不准，采购时间晚（存在库存将至0才补货的现象）；\n\t（b）存在长时间缺货现象，可能的原因是采销没有及时补货或者补货了但是供应商没有及时送到。')
        self.skuget(u'low_cr_high_td', u'（4）现货率低，周转高的SKU', u'\t这部分SKU的现货率和库存周转天数都存在改进的空间，其表现不佳一般是上述各原因综合导致的。下面给出几个此类SKU代表。', u'\t综上所述，我们认为导致现货率低或者周转天数长的主要原因有：\n\t（a）采购时间过早/过晚；\n\t（b）单次订货量过大；\n\t（c）供应商供货不稳定。')
