...
def simpleSolution():
    ...
    prob = lp.LpProblem("The Whiskas Problem", lp.LpMinimize)         # 第一个参数：是这个问题的任意名称（作为字符串），第二个参数：是lp最小化或lp最大化
    # The 2 variables Beef and Chicken are created with a lower limit of zero
    x1 = lp.LpVariable("ChickenPercent", 0, None, lp.LpInteger)       # 它有四个参数：第一个是这个变量代表的任意名字，第二个是这个变量的下界，第三个是上界，第四是数据类型（离散的或连续的）。
    x2 = lp.LpVariable("BeefPercent", 0)                           # 第二、三参数：None表示无界，正or负无穷大。第四个参数：LpContinuous（默认）/LpInteger
    # The objective function is added to 'prob' first
    prob += 0.013*x1 + 0.008*x2, "Total Cost of Ingredients per can"        # 目标函数是逻辑输入，在语句的末尾加上一个重要的逗号，以及一个简短的字符串来解释这个目标函数是什么
    # The five constraints are entered                          # 试试使用list，应该没有问题，逗号是tuple格式
    prob += x1 + x2 == 100, "PercentagesSum"                    # 添加约束
    prob += 0.100*x1 + 0.200*x2 >= 8.0, "ProteinRequirement"
    prob += 0.080*x1 + 0.100*x2 >= 6.0, "FatRequirement"
    prob += 0.001*x1 + 0.005*x2 <= 2.0, "FibreRequirement"
    prob += 0.002*x1 + 0.005*x2 <= 0.4, "SaltRequirement"
    # The problem data is written to an .lp file                # writeLP 将这些信息复制到一个.lp文件到代码块正在运行的目录中。
    prob.writeLP(r"D:\WhiskasModel.lp")
    # The problem is solved using PuLP's choice of Solver       # 指定使用哪个求解程序。prob.solve(CPLEX())
    prob.solve()
    # The status of the solution is printed to the screen       # 首先，我们要求解决方案的状态，它可以是“未解决”、“不可行的”、“无界”、“未定义”或“最优”的解决方案。
    print "Status:", lp.LpStatus[prob.status]                     # “Not Solved”, “Infeasible”, “Unbounded”, “Undefined” or “Optimal”
...
def totalSolution():
    ...
    ingredient_vars = lp.LpVariable.dicts("Ingr", Ingredients, 0)          # 变量名加了个前缀：Ingr_MUTTON，下限0
    # The objective function is added to 'prob' first
...
def shuDuSolution():
    ...
    choices = lp.LpVariable.dicts("Choice", (Vals, Rows, Cols), 0, 1, lp.LpInteger)
...
def linearMatrix():
    ...
    print(b)
    x = lp.LpVariable.matrix("x", list(range(row_num * column_num)), 0, 1, lp.LpInteger)
    print(x)
    # objective
    ...
    for count in range(column_num):
        prob += lp.lpSum(([x[i] for i in list(range(row_num * column_num)) if i % column_num == count])) == 1
        prob += lp.lpDot([x[j] for j in list(range(24)) if j % column_num == count],
                      [raw_data[j] for j in list(range(24)) if j % column_num == count]) >= 1
    # #constrain
