...
class inventory_proess:
    def __init__(self, fdc_forecast_sales, fdc_forecast_std, fdc_alt, fdc_alt_prob, fdc_inv, white_list_dict,
    ...
    def calc_sku_allocation(self, date_s):
        ...
        for s in self.union_white_list:
            ...
            #记录FDC的库存
            self.allocation_retail[date_s][s]['rdc']=self.rdc_inv[index_rdc]
            # 更新实际调拨，记录理论调拨和实际调拨
            for f in ['630', '658', '628']:
                index_fdc = self.gene_index(f, s, date_s)
                self.allocation_retail[date_s][s]['calc_'+f]=fdc_replacement[f]
                fdc_inv_avail = np.min([fdc_replacement[f], rdc_inv_avail])
                self.allocation_retail[date_s][s]['real_'+f]=fdc_inv_avail
        ...
        for d in self.date_range:
            ...
            while top_n_min_orders_retail:
                ...
                for s in dict(o[1][1]).items():
                    ...
                    #标记FDC可用库存 fdc_useage_inv=
                    # print 'self.fdc_inv[index][  ]+self.fdc_inv[index][ ]-self.fdc_inv[index][ ]'
                    # print self.fdc_inv[index]['inv'],self.fdc_inv[index]['open_po'],self.fdc_inv[index]['cons_open_po']
                    fdc_useage_inv=self.fdc_inv[index]['inv']+self.fdc_inv[index]['open_po']-self.fdc_inv[index]['cons_open_po']
                    if s[0] not in self.white_list[f]:
                        ...
                        if s[1] > self.rdc_inv[index_rdc]:
                            ...
                            if s[1] > self.rdc_inv[index_rdc] + fdc_useage_inv:
                                # print 'what happened'
                ...
                if flag_fdc == 1:
                    ...
                    for s in dict(o[1][1]).items():
                        ...
                        self.fdc_simu_orders_retail[f + d][order_index][s[0]] = self.orders_retail[f + d][order_index][s[0]]
                        # print 'fdc 的订单'
                        # print self.fdc_simu_orders_retail
                        #记录消耗在途的数量
                        if s[1]>self.fdc_inv[index]['inv']:
                            #该递推公式保证了 self.fdc_inv[index]['cons_open_po'] 小于等于  self.fdc_inv[index]['open_po']
                            #同时 在途消耗为正值，s[1]>self.fdc_inv[index]['inv'] 保证了 第一项>0
                            self.fdc_inv[index]['cons_open_po']=self.fdc_inv[index]['cons_open_po']+np.min([s[1]-self.fdc_inv[index]['inv'],
                    ...
                    for s in dict(o[1][1]).items():
                        ...
                        #订单标记与订单量标记放在第一位，因为涉及到在途消耗，该订单尚未造成实际的订到消耗增加
                        self.fdc_simu_orders_retail[f + d][order_index][s[0]] = min(self.orders_retail[f + d][order_index][s[0]],
                                                                                    self.fdc_inv[index]['inv']+self.fdc_inv[index]['open_po']-self.fdc_inv[index]['cons_open_po'])
                        self.simu_orders_retail[f + d][order_index][s[0]] = min(self.orders_retail[f + d][order_index][s[0]],self.fdc_inv[index]['inv']
                                                                            +self.fdc_inv[index]['open_po']
                                                                            -self.fdc_inv[index]['cons_open_po']
                                                                            + self.rdc_inv[rdc_index])
                        #记录消耗在途的数量
                        if s[1]>self.fdc_inv[index]['inv']:
                            #该递推公式保证了 self.fdc_inv[index]['cons_open_po'] 小于等于  self.fdc_inv[index]['open_po']
                            #同时 在途消耗为正值，s[1]>self.fdc_inv[index]['inv'] 保证了 第一项>0
                            self.fdc_inv[index]['cons_open_po']=self.fdc_inv[index]['cons_open_po']+min(s[1]-self.fdc_inv[index]['inv'],
                                                                                                        self.fdc_inv[index]['open_po']-self.fdc_inv[index]['cons_open_po'])
                        ...
                        sku_gap = s[1] - (self.fdc_inv[index]['inv']+self.fdc_inv[index]['open_po']-self.fdc_inv[index]['cons_open_po'])
