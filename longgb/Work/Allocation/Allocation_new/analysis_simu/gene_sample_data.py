...
if __name__ == '__main__':
    ...
    sample_sku_list=np.concatenate((sample_sku_list,
                   np.array(['3107131','1083590','779351','1655954',
                             ...
                             '1000054','1000274','1000564','1944463','3051749','326467','1944291','1942991'])))
    ...
    tmp_simu_orders_retail_pd=simu_orders_retail_pd['sale_qtty'].groupby([simu_orders_retail_pd["date_s"],simu_orders_retail_pd["fdc"]
                                                                             ,simu_orders_retail_pd["sku"]]).sum()
    ...
    tmp_allocation_sale_data=allocation_sale_data['sale_qtty'].groupby([allocation_sale_data["date_s"],allocation_sale_data["dc_id"]
                                                                           ,allocation_sale_data["item_sku_id"]]).sum()
    ...
    for p in date_range:
        ...
        mid_allocation_sku_data.columns= ['sku_id','mean_sales','variance','ofdsales','inv','white_flag','white_flag_01','date_s','dc_id','variance_ofdsales','std']
    ...
    tmp_03=tmp_03.loc[:,["date_s", "fdc", "sku", "inv","allocation","arrive_quantity","open_po","sale_qtty_fdc","sale_qtty"]]
    tmp_03.columns=["date_s", "fdc", "sku", "inv","allocation","arrive_quantity","open_po","sale_qtty_fdc","sale_qtty_rdc_simu"]
    allocation_sale_data=tmp_allocation_sale_data.astype(str)
    tmp_04=pd.merge(tmp_03,allocation_sale_data,left_on=['date_s','sku','fdc'],right_on=['date_s','item_sku_id','dc_id'],how='left')
    print tmp_04[tmp_04['sku']=='3107131']
    tmp_04=tmp_04.loc[:,["date_s", "fdc", "sku", "inv","allocation","arrive_quantity","open_po","sale_qtty_fdc","sale_qtty_rdc_simu","sale_qtty"]]
    #库存数据为当天的末尾数据，改为头部数据
    ...
    tmp_05=pd.merge(tmp_04,allocation_sku_data_sample,left_on=['date_s','sku','fdc'],right_on=['date_s','sku_id','dc_id'],how='left')
    print '输出相关数据'
