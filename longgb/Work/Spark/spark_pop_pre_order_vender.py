...
def main(data_path,seg_date):
    ...
    df = df.filter(df.package_dt<=seg_date).groupBy(['pop_vender_id','pop_vender_name','package_dt']).agg(sum('ord_qtty').alias('ord_qtty'))
    sqlContext = SQLContext(sc)
    threshold_sales = df.filter(df.package_dt<=seg_date).select('pop_vender_id','pop_vender_name','ord_qtty')
    threshold_sales = threshold_sales.select(threshold_sales.pop_vender_id,threshold_sales.pop_vender_name,threshold_sales.ord_qtty.astype(DoubleType()).alias('ord_qtty')).groupBy(['pop_vender_id','pop_vender_name']).agg(avg('ord_qtty').alias('sales_thre'))

    df_sales1=df.join(threshold_sales,on=['pop_vender_id','pop_vender_name'])
    df_sales2=df_sales1.select(df_sales1.pop_vender_id,df_sales1.pop_vender_name,df_sales1.package_dt,
    ...
    group_sales1=df_sales2.filter(df_sales2.package_dt<=seg_date).orderBy(['pop_vender_id','pop_vender_name','package_dt'],ascending=True).groupBy(
        ['pop_vender_id','pop_vender_name']).agg(F.collect_list('ord_qtty').alias('ord_qtty'),F.collect_list('package_dt').alias('package_dt'))
    return group_sales1

def deal_data(df):
    time_series=[0,0,df.pop_vender_id,df.pop_vender_name,0,0,df.ord_qtty]
    print time_series
