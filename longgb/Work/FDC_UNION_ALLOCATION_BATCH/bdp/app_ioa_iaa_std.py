...
#     std7 string comment '7天标准差',
#     rdc_id string) comment'FDC残差标准差'
#     partitioned by (dt string)
#     stored as TEXTFILE;
    # """

    sql = """
insert overwrite table app.app_ioa_iaa_dayerr_tmp partition (dp='"""+byesterday+"""')
select
    dt,
    sku_id,
    fdc_id,
    ...
    case when datediff(to_date('"""+ byesterday +"""'), to_date(dt)) < 1 then null
        else (
            case when forecast_daily_override_sales is null then coalesce(sales_sum3,0)
            else
            coalesce(sales_sum2,0)-forecast_daily_override_sales[0]-
            forecast_daily_override_sales[1]  end ) end day_err2,
    case when datediff(to_date('"""+ byesterday +"""'), to_date(dt)) < 2 then null
        else (
            case when forecast_daily_override_sales is null then coalesce(sales_sum3,0)
            else
    ...
    case when datediff(to_date('"""+ byesterday +"""'), to_date(dt)) < 3 then null
        else (
            case when forecast_daily_override_sales is null then coalesce(sales_sum3,0)
            else
    ...
    case when datediff(to_date('"""+ byesterday +"""'), to_date(dt)) < 4 then null
        else (
            case when forecast_daily_override_sales is null then coalesce(sales_sum3,0)
            else
    ...
    case when datediff(to_date('"""+ byesterday +"""'), to_date(dt)) < 5 then null
        else (
            case when forecast_daily_override_sales is null then coalesce(sales_sum3,0)
            else
    ...
    case when datediff(to_date('"""+ byesterday +"""'), to_date(dt)) < 6 then null
        else (
            case when forecast_daily_override_sales is null then coalesce(sales_sum3,0)
            else
...
insert overwrite table app.app_ioa_iaa_dayerr partition(dp='"""+byesterday+"""')
select
    t1.dt,
    t1.sku_id,
    t1.fdc_id,
...
insert overwrite table app.app_ioa_iaa_std partition(dt='"""+byesterday+"""')
    select
        t1.sku_id,
        t1.fdc_id,
        t1.std1,
    ...
    insert overwrite table app.app_ioa_iaa_std partition(dt='active')
    select
        sku_id,
        fdc_id,
        std1,
