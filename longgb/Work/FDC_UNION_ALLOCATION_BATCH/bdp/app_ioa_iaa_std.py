...
def main():
    ...
    # day_err7 string comment '7天预测偏差 ')
    ...
    #     lop string comment '基于3天标准差补货点 ',
    #     ti3 string comment '目标库存（3天标准差）',
    #     ti7 string comment '目标库存（7天标准差）'
    # )
    # partitioned by (dt string)
    # stored as TEXTFILE;
    # """
...
where
    dp     = '"""+end_dt+"""'
    and sku_status_cd=1
 GROUP BY sku_id,fdc_id)t2
on t1.sku_id=t2.sku_id and t1.fdc_id=t2.fdc_id;

insert overwrite table app.app_ioa_iaa_dayerr 
partition(dp='"""+end_dt+"""')
select
...
insert overwrite table app.app_ioa_iaa_std partition(dt='"""+end_dt+"""')
    ...
    forecast_daily_override_sales[0]+
    forecast_daily_override_sales[1]+
    forecast_daily_override_sales[2]+
    1.96*std3 end as lop,
    (8/7)*(forecast_daily_override_sales[0]+
    forecast_daily_override_sales[1]+
    forecast_daily_override_sales[2]+
    forecast_daily_override_sales[3]+
    forecast_daily_override_sales[4]+
    forecast_daily_override_sales[5]+
    forecast_daily_override_sales[6])+
    1.96*std3 as ti3,
    (8/7)*(forecast_daily_override_sales[0]+
    forecast_daily_override_sales[1]+
    forecast_daily_override_sales[2]+
