...
import numpy as np
import pandas as pd
from com.jd.test.zhangjs.analysis_simu.FDC_UNION_ALLOCATION.inventory_process_online import inventory_proess
from com.jd.test.zhangjs.analysis_simu.FDC_UNION_ALLOCATION import configServer
import time
...
##
#
def simulation_by_suk(sku_id,df_sku,fdc_list,write_original_data=1,test_length=30):
    # 设置仿真开始日期和0:仿真结束日期
    # 准备数据
    # print sku_id
    logger = logging.getLogger('allocation .. logger')
    ...
    # if numpy.isnan(df_sku.stock_qtty.iloc[0]):
    ...
    # 如果仿真期间总销量为0，不进行仿真
    # if abs(numpy.sum(df_sku.total_sale) - 0) <= 1e-3:
    ...
    #保存预测数据
    fdc_forecast_sales=collections.defaultdict(list)
    fdc_forecast_std=collections.defaultdict(list)
    fdc_inv=collections.defaultdict(lambda :collections.defaultdict(int))
    white_flag=collections.defaultdict(lambda :collections.defaultdict(int))
    sales_retail=collections.defaultdict(int)
    fdc_his_inv=collections.defaultdict(int)
    system_small_s=collections.defaultdict(int)
    system_bigger_S=collections.defaultdict(int)
...
for sku_id in sku_list:
    ...
    jobs.append(job_server.submit(simulation_by_suk, (sku_id,df_sku,fdc_list,1,30,), (gene_index,inventory_proess,), ('logging','configServer','ast','numpy','defaultdict','collections','copy')))
