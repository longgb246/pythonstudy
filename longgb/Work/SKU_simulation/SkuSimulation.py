...
class SkuSimulation:
    ...
    def __init__(self, date_range, sales_his, inv_his, sales_pred_mean,vlt_val, vlt_prob,s,S,ito_level=-99,
                  pred_days=28, sku_id='', sku_name='', org_nation_sale_num_band=None):
        self.sku_id = sku_id
    ...
    def calc_kpi(self):
        if self.simulation_status == 2:
            ...
            return [self.sku_id, round(cr_sim,4), round(cr_his,4), round(ito_sim,4), round(ito_his,4), round(ts_sim,4), round(ts_his,4),
                    self.pur_cnt_sim,self.s,self.S,self.date_range[0],self.date_range[1],self.ito_level,ts_rate]
        else:
