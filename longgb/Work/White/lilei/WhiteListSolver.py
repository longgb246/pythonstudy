...
class WhiteListSolver:
    ...
    def __init__(self, parent_order_id, order_sku_record, solver_path='', model_path='./', model='model.lp',
                 output='output.txt'):
    ...
    def construct_model(self, alpha=0.9):
        # 构建优化模型
        with open(self.model_path + self.model, 'a+') as f:
    ...
    def solve_model(self):
        # 调用命令行求解器
        check_output(self.solver_path + "lp_solve.exe -s " + self.model_path + self.model + ' > ' + self.model_path +
    ...
    def solution_to_file(self, path='', name='solution.csv'):
